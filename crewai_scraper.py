...
crewai_scraper.py:
 ...
 - run_once(sites, crewai_llm) — one crew per site in sites-simple.yaml
...
def fetch_html_tables(url: str) -> str:
    # fetch all <table> elements from a page, and eturn a text preview of each table
    r = _fetch(url)
    soup = BeautifulSoup(r.text, "lxml")
    tables = soup.find_all("table")
    if not tables:
        return f"NO_TABLES_FOUND at {url} — page loaded but has no <table> elements"
    ...
    return "\n\n---\n\n".join(previews) if previews else "Tables found but all empty"
...
def validate_data(scraped_text: str) -> str:
    ...
    if any(w in scraped_text for w in ["RuntimeError", "error fetching", "Timeout"]):
...
def retry_with_fallback_headers(url: str) -> str:
    ...
    """
    try:
        r = _fetch(url, headers=FALLBACK_HEADERS)
        soup = BeautifulSoup(r.text, "lxml")
        tables = soup.find_all("table")
        if tables:
            rows = []
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description = "CrewAI web scraper — hierarchical multi-agent crew",
        epilog = (
            "examples:\n"
            "  python crewai_scraper.py (same as --help)\n"
